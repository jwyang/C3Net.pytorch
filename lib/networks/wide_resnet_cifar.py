...
import math
from .resnet_cifar_analysis import BasicBlock


class Wide_ResNet_Cifar(nn.Module):

    def __init__(self, block, layers, wfactor, num_classes=100, has_selayer=False, has_gtlayer=False):
    ...
    def _make_layer(self, block, planes, blocks, stride=1):
        ...
        layers = []
        layers.append(block(self.insize, self.inplanes, planes, stride, downsample, use_se=self.has_selayer, use_gt=self.has_gtlayer))
        self.inplanes = planes * block.expansion
        for _ in range(1, blocks):
            layers.append(block(self.insize, self.inplanes, planes, use_se=self.has_selayer))
...
def wresnet20_cifar(**kwargs):
    return Wide_ResNet_Cifar(BasicBlock, [3, 3, 3], 10, **kwargs)
