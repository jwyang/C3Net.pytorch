...
class _CrossNeuronBlockInternal(nn.Module):
    def __init__(self, in_channels):
        # nblock_channel: number of block along channel axis
        # spatial_size: spatial_size
        super(_CrossNeuronBlockInternal, self).__init__()

        self.conv_in = nn.Sequential(
            nn.Conv2d(in_channels, in_channels, kernel_size=4, stride=2, padding=1, groups=in_channels, bias=True),
        ...
        self.conv_out = nn.Sequential(
            nn.ConvTranspose2d(in_channels, in_channels, kernel_size=4, stride=2, padding=1, groups=in_channels, bias=True),
            # nn.BatchNorm2d(in_channels),
            nn.ReLU(True),
            nn.ConvTranspose2d(in_channels, in_channels, kernel_size=4, stride=2, padding=1, groups=in_channels, bias=True),
            # nn.BatchNorm2d(in_channels),
...
class _CrossNeuronBlock(nn.Module):
    def __init__(self, in_channels, in_height, in_width,
        ...
        self.global_context = nn.Sequential(
            nn.Conv1d(self.spatial_area, 1, 1, 1, 0, bias=True),
            nn.ReLU(True),
        )
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, insize, inplanes, planes, stride=1, downsample=None, use_se=False, use_gt=False, reduction=8):
        ...
        if self.use_se:
            self.se = SELayer(planes, reduction=reduction)
        if self.use_gt:
            self.gt = _CrossNeuronBlockInternal(planes)
...
class ResNet_Cifar(nn.Module):
    def __init__(self, block, layers, num_classes=10, insert_layers=["1", "2", "3"], depth=1,
        ...
        self.layer1 = self._make_layer(block, 16, layers[0], has_selayer=has_selayer, has_gtlayer=has_gtlayer)
        self.layer2 = self._make_layer(block, 32, layers[1], stride=2, has_selayer=has_selayer, has_gtlayer=has_gtlayer)
        self.layer3 = self._make_layer(block, 64, layers[2], stride=2, has_selayer=has_selayer, has_gtlayer=has_gtlayer)

        if self.has_gtlayer:
            if "1" in self.layers:
                layers = []
                for i in range(self.depth):
                    layers.append(_CrossNeuronBlock(16, 32, 32, spatial_height=16, spatial_width=16, reduction=16,
            ...
            if "3" in self.layers:
                layers = []
                for i in range(self.depth):
                    layers.append(_CrossNeuronBlock(32, 16, 16, spatial_height=16, spatial_width=16, reduction=16,
                        communication=communication, enc_dec=enc_dec))
            ...
            if "4" in self.layers:
                layers = []
                for i in range(self.depth):
                    layers.append(_CrossNeuronBlock(64, 8, 8, spatial_height=8, spatial_width=8, reduction=8,
                        communication=communication, enc_dec=enc_dec))
                self.nclayer4 = nn.Sequential(*layers)
    ...
    def _make_layer(self, block, planes, blocks, stride=1, has_selayer=False, has_gtlayer=False):
        ...
        layers = []
        layers.append(block(self.insize, self.inplanes, planes, stride, downsample, use_se=has_selayer, use_gt=False))
    ...
    def forward(self, x):
        ...
        if self.has_gtlayer and "4" in self.layers:
            x = self.nclayer4(x)
            corr4_ = self._correlation(x)
        else:
            corr4_ = corr4.clone()
